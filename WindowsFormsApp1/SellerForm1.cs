...
namespace WindowsFormsApp1
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\rasya\Documents\dbMS.mdf;Integrated Security=True;Connect Timeout=30;Encrypt=False");
        ...
        private string autoID()
        {
            ...
            {
                con.Open();
                cm = new SqlCommand("SELECT COUNT(*) FROM tbPay WHERE CAST(pdate AS DATE) = CAST(GETDATE() AS DATE)", con);
                int count = Convert.ToInt32(cm.ExecuteScalar()) + 1;
                generateId = DateTime.Now.ToString("yyyyMMdd") + count.ToString("0000");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string payid = autoID(); // Generate unique ID for slid

                    if (dgvSellList.Rows.Count > 1) 
                    {
                        cm = new SqlCommand("INSERT INTO tbPay (payid) VALUES (" + payid + ")", con);
                        ...
                        foreach (DataGridViewRow row in dgvSellList.Rows)
                        {
                            ...
                            cm = new SqlCommand("SELECT * FROM tbSellList");
                            cm = new SqlCommand("INSERT INTO tbSellList(slname, sldate, pid, prname, slqty, slprice, sltotal)VALUES (@slname, @sldate, @pid, @prname, @slqty, @slprice, @sltotal)", con);
                            ...
                            //koneksi.cud("INSERT INTO OrderDetail (Id, OrderId, MenuId, Qty) VALUES('" + (koneksi.dt.Rows.Count + 1) + "','" + orderID + "',  '" + row.Cells[0].Value.ToString() + "', '" + row.Cells[2].Value.ToString() +)");
                        }
                    }
                    ...
                    dgvSellList.Rows.Add(SellerName, dateOrder.Value.ToString(), txtPname.Text, UDQty.Value.ToString(), txtPrice.Text, txtTotal.Text);
        ...
        private void LoadSellList()
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    dgvSellList.Rows[i].Cells[3].Value = dr["pid"].ToString();
                    dgvSellList.Rows[i].Cells[4].Value = dr["prname"].ToString();
                    dgvSellList.Rows[i].Cells[5].Value = dr["slqty"].ToString();
                    dgvSellList.Rows[i].Cells[6].Value = dr["slprice"].ToString();
                    dgvSellList.Rows[i].Cells[7].Value = dr["sltotal"].ToString();
                    i++;
                }
            }
            catch (Exception ex)
        ...
        private List<int> GetOrderIDsForSeller(string sellerName)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    if (int.TryParse(dr["slid"].ToString(), out int orderID))
                    {
                        orderIDs.Add(orderID);
                    }
                    else
                    {
                        // Log or handle the case where the conversion fails
                    }
                }
            }
            catch (Exception ex)
